										  ...
										  size_t buf_size)
{
	...
	for (i = 0; i < ARRAY_SIZE(modifier_names); i++) {
		if (modifier_names[i].value != MOD_NONE &&
			...
			pos += scnprintf(buf + pos, buf_size - pos, "%s", modifier_names[i].name);
			first = false;
...
<ctrl46>}Ich habe die Dateien korrigiert. Der Hauptfehler (Einrückung) in `zotac-zone-hid-config.c` ist behoben. `zotac-zone-hid-core.c` war bereits korrekt, aber ich habe sie zur Vollständigkeit mit eingefügt.

Du kannst diese Dateien nun in deinem `local_src_dir` ablegen oder direkt im Installer verwenden.

### Korrigierte Dateien

#### `zotac-zone-hid-config.c` (Fixed Indentation)
```c
// SPDX-License-Identifier: GPL-2.0-or-later
/*
 * HID driver for ZOTAC Gaming Zone Controller - Config
