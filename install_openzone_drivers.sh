...
MANAGER_LOCAL_PATH="./$MANAGER_SCRIPT_NAME"
...
print_banner() {
    ...
    echo "############################################################"
    echo "#                                                          #"
    echo "#        OPENZONE DRIVER INSTALLER (v1.5)                  #"
    echo "#                                                          #"
...
if [ ! -f "$MANAGER_LOCAL_PATH" ]; then
    ...
    if [ -f "$MANAGER_LOCAL_PATH" ]; then
        ...
        log_success "Manager downloaded."
    else
...
if [ -f "$MANAGER_LOCAL_PATH" ]; then
    ...
    read -r choice
    if [[ ! "$choice" =~ ^[Nn]$ ]]; then
        chmod +x "$MANAGER_LOCAL_PATH"#!/bin/bash
# ==============================================================================
#  ZOTAC ZONE LINUX DRIVER INSTALLER (OpenZONE)
# ==============================================================================
#  Drivers by: flukejones (Luke D. Jones)
...
if [ -f "$MANAGER_SCRIPT" ]; then
    ...
    echo -e "\n${YELLOW}Tip: Run 'openzone_manager.sh' to configure your device.${NC}"
fi
